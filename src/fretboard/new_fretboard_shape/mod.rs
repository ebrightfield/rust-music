...
use crate::fretboard::new_approach::{NewFretboard, NewFrettedNote, SoundedNote};
use crate::note::note::Note;

#[derive(Debug)]
pub struct NewFretboardShape<'a> {
    pub fretted_notes: Vec<NewFrettedNote<'a>>,
    pub fretboard: &'a NewFretboard,
}

impl<'a> Display for NewFretboardShape<'a> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let s: Vec<String> = self.fretted_notes
            .iter()
            .map(|value| {
                match value {
                    NewFrettedNote::Muted { .. } => "x".to_string(),
                    NewFrettedNote::Sounded(SoundedNote { fret, .. }) => fret.to_string(),
                }
            })
            .collect();
        let s = s.join("-");
...
impl<'a> NewFretboardShape<'a> {
        ...
        Self {
            ...
            fretted_notes: self.fretted_notes
                ...
                .map(|value| match &value {
                    NewFrettedNote::Muted { string, fretboard } => NewFrettedNote::Muted {
                        string: *string, fretboard,
                    },
                    NewFrettedNote::Sounded(
                        SoundedNote { fret: 0, string, fretboard, .. }
                    ) => NewFrettedNote::Muted {
                        string: *string, fretboard,
                    },
                    NewFrettedNote::Sounded(
                        SoundedNote { fret, pitch, string, fretboard }
                    ) => NewFrettedNote::Sounded(SoundedNote {
    ...
    pub fn size(&self) -> u8 {
        self.fretted_notes.iter()
            .fold(0, |value, item| match item {
                NewFrettedNote::Sounded(_) => value + 1,
                _ => value,
            })
    }
    ...
    pub fn span(&self) -> (u8, u8) {
        ...
        for fretted_note in &self.fretted_notes {
            if let NewFrettedNote::Sounded(SoundedNote { fret, .. }) = fretted_note {
                if *fret < lowest {
                    lowest = *fret;
                }
                if *fret > highest {
    ...
    pub fn contains_open_strings(&self) -> bool {
        self.fretted_notes.iter().any(|value| {
            match value {
                NewFrettedNote::Sounded(SoundedNote {fret: 0, ..}) => true,
                _ => false,
            }
        })
    }
    ...
    pub fn contains(&self, member: &Pitch) -> bool {
        for fretted_note in &self.fretted_notes {
            if let NewFrettedNote::Sounded(SoundedNote { pitch, ..}) = fretted_note {
    ...
    pub fn classify(&self) -> ChordShapeClassification {
        if self.is_playable() {
            if self.fretted_notes.iter().all(|value| {
                match value {
                    NewFrettedNote::Sounded(SoundedNote { fret, .. }) => *fret > 12,
                    _ => true,
                }
            }) {
...
impl<'a> From<&'a NewFretboardShape<'a>> for StackedIntervals {
    fn from(value: &'a NewFretboardShape<'a>) -> Self {
        let mut pitches: Vec<Pitch> = value
            .fretted_notes
            .iter()
            .map(|fretted_note| match &fretted_note {
                NewFrettedNote::Sounded(SoundedNote { pitch, .. }) => {
...
#[cfg(test)]
mod tests {
    use crate::fretboard::new_fretboard_shape::chord_shape_search::find_chord_shapes;
