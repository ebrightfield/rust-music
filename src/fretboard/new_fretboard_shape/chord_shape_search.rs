...
use crate::fretboard::new_approach::{NewFretboard, NewFrettedNote};
use crate::fretboard::new_fretboard_shape::{ChordShapeClassification, NewFretboardShape};
use crate::note::note::Note;

#[derive(Debug)]
pub struct ChordShapeSearchResult<'a> {
    pub playable: HashMap<StackedIntervals, Vec<NewFretboardShape<'a>>>,
    pub wide_intervals: HashMap<StackedIntervals, Vec<NewFretboardShape<'a>>>,
    pub nontransposable: HashMap<StackedIntervals, Vec<NewFretboardShape<'a>>>,
    pub all_above_12th_fret: HashMap<StackedIntervals, Vec<NewFretboardShape<'a>>>,
    pub unplayable: HashMap<StackedIntervals, Vec<NewFretboardShape<'a>>>,
}
...
pub fn find_chord_shapes<'a>(chord: &Vec<Note>, fretboard: &'a NewFretboard) -> anyhow::Result<ChordShapeSearchResult<'a>> {
                ...
                let strings = (0u8..num_strings)
                    .map(|i| {
                        ...
                        if let Some(index) = index {
                            return Ok::<_, anyhow::Error>(NewFrettedNote::Sounded(
                                fretboard.at(i, *fret_shape[index])?
                            ));
                        }
                        Ok::<_, anyhow::Error>(NewFrettedNote::Muted {
                            string: i,
                            fretboard,
                        })
                    })
